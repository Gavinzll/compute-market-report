...
#### 7.4.1 偏差率硬拦截（Deviation Hard Reject）

当国内月租与海外同型号等效 8 卡月租可对比时，计算偏差率 = 国内月租 ÷ 海外月租 × 100%。偏差率超过阈值的数据**无论置信度多高**，立即 `REJECT`，不得进入主图、主指数、ROI 或 AI 总结，并写入 `rejected` 文件、注明 reject 原因：

- **训练卡**（B300/B200/GB200/GB300/H200/H100/H800/H20/A100/A800 等）：偏差率 > 500% → `REJECT`
- **消费级卡**（RTX 5090/4090 等）：偏差率 > 1000% → `REJECT`
- **统一兜底**：任何 GPU 偏差率 > 1000% → `REJECT`

正常国内/海外月租比通常在 30%-200% 区间。超阈值几乎可断定为采集失真、单位错配（如把"元"当"万元"、把单卡价当整机价）或口径混淆。置信度高不等于数据正确——**偏差率硬拦截优先于置信度**，是防止高置信失真数据进入报告的最后一道闸门。

> 反面案例（2026-07-31）：RTX 5090 采集到 "SMM直播：500.0万/月"，置信度 85 通过了 7.1 置信度校验，但偏差率 12953%（海外仅 2.93 万/月），属于把单卡小时价或元级报价误当万元级整机月租的典型单位错配。本规则即为此设立。
...
本节约束 CMIS 价格指数（`scripts/cmis_index.py`）的构建与披露，完整方法论见 `prompts/methodology.md`；两者冲突时以本节与本文档为准。

- **指数值来源**：指数值只能由 `Validate == PASS` 的标准化价格形成（国产战略关注卡按第 0 节规则进入展示但不形成 PASS 指数）。单源时直接采用并披露源数量；同型号存在多条**同口径、同分段** PASS 报价时，按 Confidence 加权平均聚合，并以中位数交叉核对（偏差 > 20% 标记复核）。聚合方式、各源原值与权重必须写入审计文件。
- **与来源优先级的关系**：第 7.5 节来源优先级仍是冲突裁决规则——低优先级源与高优先级源冲突时采用高优先级，不得平均。本节的加权聚合只适用于**同优先级层级内**的多源确认（对应 Source Consensus 为 High/Medium 的场景），不得用聚合稀释高优先级来源。
- **市场分段披露**：海外读数按 NeoCloud / Hyperscaler / Aggregator / Marketplace 分段标注，两段读数分开发布，**禁止**混合为单一"全市场价"。采集端保留分供应商逐条报价（`provider_quotes`），具备两段样本时计算并展示分段价差；样本不足时该模块不展示，不得编造。
- **覆盖率披露**：每期必须实算并写入审计与报告指数区——国内/海外基线 GPU 指数覆盖比例、海外数据源数、Token 模型覆盖比例。只报真实值，不得把 REVIEW / 云价折算样本计入覆盖率。
- **期限结构纪律**：采集海外报价必须保留 `contract_period`；reserved/committed 与 on-demand 是不同基差，不得混合。**远期曲线、价格预测类内容在未积累 ≥3 个真实期限点之前一律不发布**，禁止用公式推导冒充市场远期价。
- **指数快照**：指数计算结果（含 `coverage`、`segment_spread`）写入 `data/cmis_snapshot_{YYYY-MM-DD}.json` 的 `cmis_indices` 字段，作为历史趋势与审计依据。
