...
- `Main Index`：普通样本只纳入 `Validate == PASS` 且 `Confidence >= 70` 的标准口径数据，用于主图、主指数、ROI、历史库和 AI 总结。国产战略关注卡例外：`昇腾 910B`、`寒武纪 MLU`、`海光 DCU`、`壁仞`、`摩尔线程` 必须进入国内指数表和国内柱状图；若标准 8卡整机长租价不可得，必须用低置信 8卡整机战略观察价占位，并标为 `REVIEW / Strategic Watch`，不进入 ROI、历史方向判断或 AI 方向性结论。严禁把云主机单卡价、单卡月租或未确认卡数的套餐价直接乘以 8 当作 8卡整机月租。
...
**硬性规则：Confidence < 70 的普通数据不得进入主指数。** 可进入辅助模块、异常观察或 Rejected Samples。国产战略关注卡例外：`昇腾 910B`、`寒武纪 MLU`、`海光 DCU`、`壁仞`、`摩尔线程` 即使 Confidence < 70 也必须列入国内指数表和国内柱状图；价格不足时用低置信 8卡整机战略观察价占位，并标注 `战略关注/待复核`、来源、架构/卡数不确定性和折算方式，且不得进入 ROI、历史方向判断或 AI 方向性结论。Confidence Score 必须写入审计文件。
