...
- `Main Index`：普通样本只纳入 `Validate == PASS` 且 `Confidence >= 70` 的标准口径数据，用于主图、主指数、ROI、历史库和 AI 总结。国产战略关注卡例外：`昇腾 910B`、`寒武纪 MLU`、`海光 DCU`、`壁仞`、`摩尔线程` 必须进入国内指数表展示；若置信度不足或价格未量化，标为 `REVIEW / Strategic Watch`，不进入 ROI、历史方向判断或 AI 方向性结论。
...
**硬性规则：Confidence < 70 的普通数据不得进入主指数。** 可进入辅助模块、异常观察或 Rejected Samples。国产战略关注卡例外：`昇腾 910B`、`寒武纪 MLU`、`海光 DCU`、`壁仞`、`摩尔线程` 即使 Confidence < 70 也必须列入国内指数表，但必须标注 `战略关注/待复核`，且不得进入 ROI、历史方向判断或 AI 方向性结论。Confidence Score 必须写入审计文件。
