...
**硬性规则：Confidence < 70 的普通数据不得进入主指数。** 可进入辅助模块、异常观察或 Rejected Samples。国产战略关注卡例外：`昇腾 910B`、`寒武纪 MLU`、`海光 DCU`、`壁仞`、`摩尔线程` 即使 Confidence < 70 也必须列入国内指数表和国内柱状图。每日运行必须先搜索这些卡的新资料：SMM、运营商/IDC、国产智算中心、云厂商包年包月、8卡训推一体机租赁、集成商公开报价、招投标和可公开访问的媒体/社区线索。若找到明确的 8卡整机/裸机/月租市场价，应按来源置信度更新价格、价格口径、区间和备注；若没有新价，才沿用上一版 `市场核价区间（估算）` 或 `云价折算` fallback。市场核价必须标注为估算、写明区间和依据、不得冒充公开成交价、不得进入 ROI 或方向性结论。若连配置/卡数/形态都无法确认，才显示 `价格待补`；严禁用 910B 区间、置信度或无来源系数生成占位价。Confidence Score 必须写入审计文件。
...
Token 三方价必须分为海外三方价和境内三方价，并额外输出 `三方价格匹配级别` 与 `同价原因`。匹配级别必须保守标注：精确同名、官方同步/路由价、同厂参考、同系列参考、同类型参考、近似参考；凡含参考、待补、近似、同厂、同系列、同类型、非文本精确价等口径，不得标为精确同名。官方价与三方价完全相等时，必须解释是第三方同步官方价、同名模型当前价一致，还是参考价巧合相等。
...
采购价分层优先级为：官方公开价/招投标/中标公告 > 框架协议/运营商/银行集采 > 整机厂商或渠道公开报价 > 可公开访问的媒体/社区线索 > 市场核价区间（估算）。如果无法取得精确采购价，但已经确认 GPU 型号、8卡整机形态或单卡公开价，可以形成采购价市场核价区间；必须写明采购价区间、中位数、依据来源、置信度和不可审计边界，不得冒充公开成交价。
