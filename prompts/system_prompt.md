...
**硬性规则：Confidence < 70 的普通数据不得进入主指数。** 可进入辅助模块、异常观察或 Rejected Samples。国产战略关注卡例外：`昇腾 910B`、`寒武纪 MLU`、`海光 DCU`、`壁仞`、`摩尔线程` 即使 Confidence < 70 也必须列入国内指数表和国内柱状图。若无公开成交价但已经确认硬件配置、卡数或 8卡整机/训推一体机形态，可以基于公开配置、云价、行业研报、可比 8卡裸机价格、集成商/智算中心线索形成 `市场核价区间（估算）`，但必须标注为估算、写明区间和依据、不得冒充公开成交价、不得进入 ROI 或方向性结论。若连配置/卡数/形态都无法确认，才显示 `价格待补`；严禁用 910B 区间、置信度或无来源系数生成占位价。Confidence Score 必须写入审计文件。
