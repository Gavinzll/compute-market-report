...
国产战略关注例外：`昇腾 910B`、`寒武纪 MLU`、`海光 DCU`、`壁仞`、`摩尔线程` 必须进入国内指数表和国内柱状图。若无法取得标准 8卡整机月租或 Confidence < 70，仍保留在指数和柱状图中并标注 `Strategic Watch / REVIEW`。每日刷新优先级为：明确公开市场价 > 可折算云价/包年包月价 > 市场核价区间 fallback > 价格待补。每次运行都要先扩源到国产智算中心报价、8卡训推一体机租赁、集成商报价、媒体/社区线索和招投标线索；若找到新的明确 8卡整机/裸机/月租，应更新 `价格口径`、`核价区间`、`标准化价格`、`Confidence Score` 和备注。严禁把云主机单卡价、单卡月租或未确认卡数的套餐价直接乘以 8 当成 8卡整机月租；只有同时确认硬件配置/卡数和公开价格时，才允许折算为 `云价折算` 或公开价。若无公开成交价但已确认配置、卡数或 8卡整机形态，可以形成或沿用 `市场核价区间（估算）`：必须写明估算区间、区间中位数、依据来源、不可审计边界，并在图表标签标注 `市场核价`；不得把估算冒充公开成交价，不得进入 ROI 或 AI 方向性结论。连配置/卡数/形态都无法确认时才显示 `价格待补`，不得用 910B 区间或任何无来源系数生成占位价。
...
| 天翼云 PCH1 | ❌ 无 | ❌ 无 | ❌ 无 | ✅ MLU370-S4有云价 | ❌ 无 | 历史采集记录 |

**关键结论：** 截至 2026-07-15，极智算和云擎天下**仅覆盖910B和NVIDIA系列**，尚未在公开页面找到摩尔线程MTT S4000、壁仞BR100/BR106、寒武纪MLU590、海光DCU Z100的租赁价格。对这些国产卡的扩源应优先尝试：
1. 曙光智算 scnet.cn（登录后查看资源市场→裸金属→国产DCU/MLU）
2. SMM 算力快讯持续跟踪（关键词：`寒武纪 MLU590 月租`、`海光 DCU Z100 租赁`、`壁仞 BR100 月租`、`摩尔线程 MTT S4000 租赁`）
3. 天翼云/华为云/阿里云国产算力专区（云价折算来源）
4. 算力社群/微信群（低置信线索，仅作Candidate）
5. 淘宝/闲鱼搜索（关键词：`寒武纪MLU590 裸金属`、`海光DCU Z100 服务器租赁`）
