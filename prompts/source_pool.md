...
国产战略关注例外：`昇腾 910B`、`寒武纪 MLU`、`海光 DCU`、`壁仞`、`摩尔线程` 必须进入国内指数表和国内柱状图。若无法取得标准 8卡整机月租或 Confidence < 70，仍保留在指数和柱状图中并标注 `Strategic Watch / REVIEW`；价格不足时继续扩源到国产智算中心报价、8卡训推一体机租赁、集成商报价、媒体/社区线索和招投标线索。严禁把云主机单卡价、单卡月租或未确认卡数的套餐价直接乘以 8 当成 8卡整机月租；只有同时确认硬件配置/卡数和公开价格时，才允许折算。缺少价格依据时必须显示 `价格待补`，不得用 910B 区间或任何系数生成占位价。
