...
3. 读取：
   ...
   - `prompts/methodology.md`（指数方法论；与 system_prompt.md 冲突时以后者为准）
...
6. 运行基础校验：
   ...
   - **指数自检**：确认 `data/cmis_snapshot_{YYYY-MM-DD}.json` 的 `cmis_indices` 含 `gpu_cn` / `gpu_us` / `token` / `coverage` 四组，且 `gpu_us` 非空、`coverage.gpu_cn` / `coverage.gpu_us` 为实算数值；报告指数区应展示海外指数卡与指数覆盖率行。缺失说明 `cmis_index.py` 调用被绕过，需回查生成脚本。
