...
国产战略关注卡必须进入国内指数表和国内柱状图：`昇腾 910B`、`寒武纪思元 MLU`、`海光 DCU`、`壁仞`、`摩尔线程`。这些卡即使 Confidence < 70 或仍处于 REVIEW，也不得从国内指数和柱状图中移除。若标准 8卡整机长租价不可得，必须继续扩源到国产智算中心报价、8卡训推一体机租赁、集成商报价、媒体/社区线索和招投标线索。价格每天动态刷新：如果搜到新的明确 8卡整机/裸机/月租市场价，就用新价更新表格和图表；如果没有新价，则沿用上一版市场核价区间或云价折算 fallback。只有同时具备“硬件配置/卡数”与“公开价格”两个要素时，才允许折算为 8卡月租；不得把云主机单卡价、单卡月租或未确认卡数的套餐价直接乘以 8 当成 8卡整机月租。云实例价格可以折为 `8卡云实例等效价`，但必须显著标注 `云价折算`，不得伪装成 8卡整机长租成交价。若无公开成交价但已经确认配置、卡数或 8卡整机/训推一体机形态，可以展示 `市场核价区间（估算）`，并用区间中位数进入柱状图；表格必须展示区间、依据和“非公开成交价”说明，图表标签必须标注 `市场核价`。国内柱状图必须按 `公开成交/主口径价`、`低置信观察`、`云价折算`、`市场核价`、`价格待补` 分色并显示图例。市场核价不得进入 ROI、采购回本测算或 AI 方向性结论。连配置/卡数/形态都无法确认时才显示 `价格待补`，不得编造低置信占位价或系数价。
...
- 对 `昇腾 910B`、`寒武纪思元 MLU`、`海光 DCU`、`壁仞`、`摩尔线程`，即使扩源后 Confidence 仍不足，也必须作为国产战略关注样本列入国内指数表，并标注 `战略关注/待复核`。
