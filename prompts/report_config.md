...
国产战略关注卡必须进入国内指数表展示：`昇腾 910B`、`寒武纪思元 MLU`、`海光 DCU`、`壁仞`、`摩尔线程`。这些卡即使 Confidence < 70 或仍处于 REVIEW，也不得从国内指数表中移除；若价格尚未量化，标准化价格可显示 `暂不可得`，但必须保留扩源状态、置信度、Review 原因和下一步。图表只绘制已有可量化月租的国产战略关注样本；未量化样本不进入柱状图、ROI、历史方向判断或 AI 方向性结论。
