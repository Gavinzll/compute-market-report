...
SOURCES = [
    ...
    },
    {
        "id": 70,
        "tier": "国产租赁/平台",
        "title": "极智算 jygpu.com：910B 裸金属租赁",
        "url": "https://www.jygpu.com/",
        "note": "已验证有 910B 裸金属（15,600-17,040元/月）和 NVIDIA 系列；尚未验证摩尔线程/壁仞/寒武纪/海光覆盖。",
    },
    {
        "id": 71,
        "tier": "国产租赁/平台",
        "title": "云擎天下 omniyq.com：GPU 算力批量租赁",
        "url": "https://www.omniyq.com/",
        "note": "已验证有 910B 裸金属（15,000元/月）和 NVIDIA 系列；尚未验证摩尔线程/壁仞/寒武纪/海光覆盖。",
    },
    {
        "id": 72,
        "tier": "国产云价/单卡",
        "title": "胜算云 shengsuanyun.com：国产 GPU 单卡云实例",
        "url": "https://www.shengsuanyun.com/hashrate",
        "note": "已验证有壁仞天垓100（1.49元/时）、摩尔线程S4000（1.69元/时）、昇腾910（2.36元/时）单卡云实例价；无8卡整机月租价。",
    },
    {
        "id": 73,
        "tier": "国产云价/单卡",
        "title": "模力方舟 ai.gitee.com/compute：Gitee AI 算力市场",
        "url": "https://ai.gitee.com/compute",
        "note": "已验证有海光BW1000（3.00元/时）、摩尔线程S5000（8.00元/时）、壁仞天垓150（3.00元/时）、壁砺106M（2.00元/时）、天数智芯智铠100（2.00元/时）、燧原S60（2.00元/时）、昇腾910B（3.00-4.00元/时）单卡云实例价；无8卡整机月租价。",
    },
    {
        "id": 74,
        "tier": "国产租赁/平台",
        "title": "智星云 ai-galaxy.cn：GPU 算力服务平台",
        "url": "https://www.ai-galaxy.cn/",
        "note": "有昇腾910B、海光DCU、壁仞天垓100现货，但公开页面无价格；裸金属/云主机需登录查看。",
    },
    {
        "id": 75,
        "tier": "国产租赁/门户线索",
        "title": "什么值得买：区域 GPU 租用调研",
        "url": "https://post.m.smzdm.com/",
        "note": "厦门地区调研显示寒武纪思元590单卡7,000-8,000元/月、昇腾910B单卡8,500-9,500元/月；非8卡整机价，仅作区域市场线索。",
    },
]
...
def _load_discovered_benchmark() -> list[dict] | None:
    """尝试加载动态发现的 Benchmark 评测数据；失败时返回 None。"""
    path = ROOT / "data" / f"discovered_benchmark_{DATE}.json"
    try:
        if not path.exists():
            return None
        data = json.loads(path.read_text(encoding="utf-8"))
        ...
        print(f"[warn] failed to load discovered benchmark: {e}")
    return None
...
BENCHMARK_DATA = _load_discovered_benchmark() or _BENCHMARK_FALLBACK

BENCHMARK_COLUMNS = [
    "厂商", "模型", "AA Intelligence Index", "AA Coding Index",
    "GDPval-AA v2 Elo", "Output Speed (t/s)", "Cost per Task (USD)",
    "来源", "更新时间"
]
...
DOMESTIC_RENTAL_INPUT = {
    ...
    "昇腾 910B": {
        "original": "SMM 样本：910B2 月租约 1.2-1.5 万元；极智算 8×910B 裸金属 15,600-17,040 元/月；云擎天下 8×910B 裸金属 15,000 元/月；模力方舟 910B 单卡云实例 3.00-4.00 元/时（非8卡整机价）",
        "monthly_wan": 1.35,
        "price_basis": "低置信观察",
        "price_band": "1.2-1.5（SMM）；1.5-1.7（极智算/云擎天下）",
        "price_refresh_rule": "每日优先检索 SMM、极智算 jygpu.com、云擎天下 omniyq.com、模力方舟 ai.gitee.com/compute、运营商、IDC、集成商和国产智算中心明确 8卡整机月租；若出现更高置信公开价，替换当前 SMM 区间中位数；否则沿用低置信观察价并保持 REVIEW。",
        "source": "SMM 算力直播 + 极智算/云擎天下 910B 裸金属价（多源交叉验证）",
        "confidence": 75,
        "consensus": "Medium",
        "historical": "HIST_INSUFFICIENT",
        "status": "REVIEW",
        "note": "910B 子型号、形态和异地部署限制未拆清；极智算和云擎天下已验证有 910B 8卡裸金属价（1.5-1.7 万/月），与 SMM 区间 1.2-1.5 万/月部分重叠。模力方舟有 910B 单卡云实例价（3-4元/时），但无8卡整机月租。因属于国产战略关注卡，按 SMM 区间中点进入国内指数展示，但需继续扩源确认子型号差异，不进入 ROI。",
    },
    "寒武纪 MLU370-X8": {
        "original": "天翼云 PCH1 寒武纪云主机最大公开规格 pch1.21xlarge.3：84 vCPU / 252GB / 4×MLU370-S4 / 包月 24964.07 元；折算 8 卡云实例等效价=24964.07×2×0.85≈4.24 万元/月（国产计算加速型云主机 1-3 年 8.5 折）。极智算、云擎天下公开页面未找到寒武纪 MLU 租赁价。什么值得买区域调研显示厦门地区思元590单卡 7,000-8,000 元/月（非8卡整机价）。",
        "monthly_wan": 4.24,
        "price_basis": "云价折算",
        "price_band": "云价折算 4.24",
        "price_refresh_rule": "每日优先检索寒武纪 MLU 8卡整机/裸机长租公开价（SMM、曙光智算 scnet.cn、天翼云、运营商、IDC、什么值得买区域调研）；若找到明确市场价，用公开市场价替代云价折算；若未找到，继续使用天翼云 4卡云实例折算价。关键词：寒武纪 MLU590 月租、MLU370-X8 租赁。",
        "source": "天翼云 PCH1：4×MLU370-S4 云主机包月价折算为 8卡云实例等效价（非8卡整机长租成交价）",
        "confidence": 62,
        "consensus": "Low",
        "historical": "HIST_INSUFFICIENT",
        "status": "REVIEW",
        "note": "寒武纪 MLU 属于国产战略关注卡；当前入图价格基于天翼云明确 4卡云主机配置与包月价折算到 8卡云实例等效价，不是8卡整机长租成交价。截至2026-07-15，寒武纪MLU590无任何公开8卡整机月租；什么值得买仅发现思元590单卡区域数据（7,000-8,000元/月）。需继续扩源至曙光智算、SMM 算力快讯。不进入 ROI。",
    },
    "海光 DCU K100": {
        ...
        "historical": "HIST_INSUFFICIENT",
        "status": "REVIEW",
        "note": "海光 DCU 属于国产战略关注卡；截至2026-07-15，模力方舟有 BW1000 单卡云实例价（3.00元/时），但无 DCU Z100 或 BW1000 的8卡整机月租；天翼云仅有 DCU-K100。当前价格基于单卡云价×8折算，含0.7长协折扣系数，仅作量级参考，非成交价。不进入 ROI。",
    },
    "壁仞 BR100": {
        ...
        "historical": "HIST_INSUFFICIENT",
        "status": "REVIEW",
        "note": "壁仞 BR100 属于国产战略关注卡；截至2026-07-15，胜算云有天垓100单卡云实例（1.49元/时），模力方舟有天垓150（3.00元/时）和壁砺106M（2.00元/时）单卡价，但均无 BR100 OAM服务器8卡整机月租。当前价格基于天垓100单卡云价×8折算，含0.7长协折扣系数，仅作量级参考，非成交价。不进入 ROI。",
    },
    "摩尔线程 MTT S4000": {
        ...
        "historical": "HIST_INSUFFICIENT",
        "status": "REVIEW",
        "note": "摩尔线程 MTT S4000 属于国产战略关注卡；截至2026-07-15，胜算云有 S4000 单卡云实例（1.69元/时），模力方舟有 S5000 单卡（8.00元/时），但均无8卡整机月租。当前价格基于 S4000 单卡云价×8折算，含0.7长协折扣系数，仅作量级参考，非成交价。不进入 ROI。",
    },
    "寒武纪 MLU590": {
        ...
        "price_basis": "价格待补",
        "price_band": None,
        "price_refresh_rule": "每日检索 SMM 算力快讯、曙光智算 scnet.cn、天翼云、什么值得买区域调研、算力社群、招投标网站、淘宝/闲鱼；关键词：寒武纪 MLU590 月租、MLU590 8卡 整机 租赁、思元590 服务器。若找到明确 8卡整机月租，立即更新；否则保持价格待补。",
        "source": "未采集到公开8卡整机月租",
        "confidence": 25,
        "consensus": "Low",
        "historical": "HIST_INSUFFICIENT",
        "status": "REVIEW",
        "note": "寒武纪 MLU590 属于国产战略关注卡；截至2026-07-15 无任何公开 8卡整机月租价。什么值得买仅有思元590单卡区域数据（7,000-8,000元/月），SMM 尚未覆盖 MLU590。需继续扩源至曙光智算登录后查看、算力社群、招投标线索。不进入 ROI。",
    },
    "海光 DCU Z100": {
        "original": "截至2026-07-15，未找到海光 DCU Z100 任何公开 8卡整机月租价。模力方舟仅有海光 BW1000 单卡云实例 3.00 元/时（非8卡整机价）；天翼云仅有 DCU-K100 云主机价，无 DCU Z100。",
        "monthly_wan": None,
        "price_basis": "价格待补",
        "price_band": None,
        "price_refresh_rule": "每日检索 SMM 算力快讯、曙光智算 scnet.cn、天翼云、模力方舟 ai.gitee.com/compute、算力社群、招投标网站、淘宝/闲鱼；关键词：海光 DCU Z100 月租、DCU Z100 8卡 整机 租赁、海光 GPU 服务器。若找到明确 8卡整机月租，立即更新；否则保持价格待补。",
        "source": "未采集到公开8卡整机月租",
        "confidence": 20,
        "consensus": "Low",
        "historical": "HIST_INSUFFICIENT",
        "status": "REVIEW",
        "note": "海光 DCU Z100 属于国产战略关注卡；截至2026-07-15 无任何公开 8卡整机月租价。模力方舟有 BW1000 单卡云实例（3.00元/时），但无 DCU Z100 租赁；天翼云仅有 DCU-K100。需继续扩源至曙光智算登录后查看、算力社群、招投标线索。不进入 ROI。",
    },
}
...
def domestic_rows() -> list[dict]:
    ...
    for idx, gpu in enumerate(GPU_ORDER, 1):
        ...
        # 动态数据可能缺少 price_refresh_rule，从硬编码补齐
        if gpu in _DYNAMIC_DOMESTIC and gpu in DOMESTIC_RENTAL_INPUT:
            for aux_key in ("price_refresh_rule",):
...
PROCUREMENT_INPUT = {
    "H100 80G": {"purchase_min_wan": 230, "purchase_mid_wan": 275, "purchase_max_wan": 320, "basis": "采购价估算", "source": "BIZON H100/H200 配置价、公开 8卡 GPU 服务器中标样本、国内渠道核价", "confidence": 58, "status": "REVIEW", "note": "H100 8卡 HGX 国内采购价公开成交样本稀缺；本期按海外整机配置价、国内招投标通用 8卡服务器样本和渠道核价给出 230-320 万元区间。"},
    "H20": {"purchase_min_wan": 100, "purchase_mid_wan": 120, "purchase_max_wan": 140, "basis": "三方渠道价/估算", "source": "超聚变 8U AI 8卡 H20 服务器渠道价约 109 万元；结合 H20 141GB 供需溢价核价", "confidence": 55, "status": "REVIEW", "note": "H20 8卡整机存在公开渠道价线索，但非政府中标或厂商官方报价；按 100-140 万元区间进入参考测算。"},
    "A100 80G": {"purchase_min_wan": 90, "purchase_mid_wan": 115, "purchase_max_wan": 140, "basis": "采购价估算", "source": "BIZON A100 80GB 配置价、国内 8卡 GPU 服务器中标样本、存量卡渠道价", "confidence": 55, "status": "REVIEW", "note": "A100 80G 已进入存量/二级市场，公开 8卡整机成交价分散；按 90-140 万元区间做采购成本参考。"},
    ...
    "昇腾 910C": {"purchase_min_wan": 170, "purchase_mid_wan": 220, "purchase_max_wan": 270, "basis": "采购价估算", "source": "昇腾 910B 框架协议价、910C 供需溢价和国产高端训练服务器渠道估算", "confidence": 48, "status": "REVIEW", "note": "910C 明确 8卡整机采购成交价仍少；按 910B 框架协议训练服务器价格带上修估算，需继续追踪运营商和银行集采。"},
    "昇腾 910B": {"purchase_min_wan": 155, "purchase_mid_wan": 190, "purchase_max_wan": 225, "basis": "框架协议线索/估算", "source": "农业发展银行国产 GPU 服务器框架协议线索：910B 推理服务器 154.5 万元/台、训练服务器 212.2 万元/台、配件约 9.7 万元", "confidence": 52, "status": "REVIEW", "note": "910B 采购价有框架协议线索，但原始公告仍需追溯；租赁端价格受 B2/B3/B4 子型号、库存和生态迁移影响，毛回本仅作风险观察。"},
    "寒武纪 MLU370-X8": {"purchase_min_wan": 20, "purchase_mid_wan": 30, "purchase_max_wan": 40, "basis": "三方渠道价/估算", "source": "MLU370-S4/X8 公开卡价与国产 8卡整机渠道估算", "confidence": 40, "status": "REVIEW", "note": "寒武纪 MLU370-X8 当前租赁价为云价折算，采购价按 MLU370 卡价和整机集成成本估算；二者口径不同，毛回本不进入 ROI 结论。"},
    "海光 DCU K100": {"purchase_min_wan": 60, "purchase_mid_wan": 78, "purchase_max_wan": 95, "basis": "公开卡价/估算", "source": "海光 K100 64GB GPU 卡高校采购单价 4.6 万元、公开渠道 8.4 万元/张，加服务器底座和集成成本估算", "confidence": 56, "status": "REVIEW", "note": "海光 DCU K100 采购价按单卡公开价×8加整机底座、网络、电源、维保和集成成本估算；需继续补 8卡整机中标价。"},
    "壁仞 BR100": {"purchase_min_wan": 60, "purchase_mid_wan": 80, "purchase_max_wan": 100, "basis": "采购价估算", "source": "壁仞 BR100/BR104 8卡 OAM 服务器形态、国产高端训练卡可比价格、渠道估算", "confidence": 35, "status": "REVIEW", "note": "未取得壁仞 BR100 8卡整机公开采购价；按已确认 8卡 OAM 服务器形态和国产训练卡可比区间估算，需重点扩源。"},
    "摩尔线程 MTT S4000": {"purchase_min_wan": 65, "purchase_mid_wan": 80, "purchase_max_wan": 95, "basis": "公开卡价/估算", "source": "MTT S4000 公开报价约 5.9-8.64 万元/张，叠加 8卡服务器底座、网络、供电和维保估算", "confidence": 52, "status": "REVIEW", "note": "摩尔线程 MTT S4000 8卡整机采购价按公开单卡价和整机集成成本估算；S5000/SGX5000 需继续查官方和集成商报价。"},
}
...
def mobile_benchmark_cards(rows: list[dict]) -> str:
    """手机端评测排行卡片：按 Intelligence Index 从高到低排列。"""
    sorted_rows = sorted(rows, key=lambda r: r.get("AA Intelligence Index", 0) or 0, reverse=True)
    cards = []
    for row in sorted_rows:
        intel = row.get("AA Intelligence Index", 0) or 0
        coding = row.get("AA Coding Index", 0) or 0
        # Intelligence Index 为 0 表示无数据，显示为"暂无"
        intel_str = str(intel) if intel > 0 else "暂无"
        cards.append(f"""
        <article class="m-card compact">
          <div class="m-card-head"><div><h3>{html_escape(row.get("厂商", ""))}</h3><small>{html_escape(row.get("模型", ""))}</small></div><span>{html_escape(row.get("来源", ""))}</span></div>
          <div class="pill-row">
            {pill("智能指数", intel_str)}
            {pill("编程指数", coding if coding > 0 else "暂无")}
            {pill("Elo", row.get("GDPval-AA v2 Elo", 0) or 0)}
            {pill("$/Task", row.get("Cost per Task (USD)", 0) or 0)}
          </div>
        </article>""")
    return "\n".join(cards)
...
def market_summary_html() -> str:
    ...
    return (
        f"<p>本期国内算力租赁指数共展示 {len(domestic_index_rows)} 个型号，其中 {len(pass_rows)} 个为 PASS 主口径样本，{h100_text}；国产战略关注卡继续保留在指数和图表中，但低置信和云价折算样本均标注为观察口径。</p>"
        ...
        f"<p>当前需要重点复核的是 {len(strategic_review)} 个国产战略关注样本：若后续采集到明确 8卡整机/裸机/月租或采购价，应优先用新公开价格替换云价折算；未取得新价前，不进入 ROI 或方向性市场结论。</p>"
    )
...
<body>
  <main class="page">
    ...
    <section id="model-benchmark">
      <h2>模型能力评测排行</h2>
      <p class="note">评测数据来自 Artificial Analysis Intelligence Index v4.1（综合 9 项评测），覆盖与 Token 价格表相同的模型。Intelligence Index 越高表示综合智能越强；Coding Index 衡量编程能力；GDPval-AA v2 Elo 衡量实际工作价值。更新频率约 2-3 天/次，以来源标注日期为准。</p>
    ...
    <section id="ai-summary">
      <h2>AI 总结</h2>
      <div class="panel">
        <p>本期国内服务器价格覆盖继续扩大：H100 80G 约 7.6 万元/8卡整机/月，A100 80G 约 3.15-3.8 万元/月，H20 141GB 约 4.8 万元/月，RTX 5090 约 1.2 万元/月，RTX 4090 约 0.68-0.88 万元/月，昇腾 910C 行业均价约 6.2 万元/月。昇腾 910B、寒武纪 MLU370-X8、海光 DCU K100、壁仞 BR100、摩尔线程 MTT S4000已作为国产战略关注卡进入国内指数表和柱状图；寒武纪使用天翼云实例折算口径，海光/壁仞/摩尔线程使用单卡云价×8折算展示，均不进入 ROI。</p>
      </div>
    </section>
...
def write_charts():
    ...
    # Benchmark 评测图表数据（按 Intelligence Index 从高到低排序）
    _bench_sorted = sorted(BENCHMARK_DATA, key=lambda r: r.get("AA Intelligence Index", 0) or 0, reverse=True)
    _bench_sorted = [r for r in _bench_sorted if (r.get("AA Intelligence Index", 0) or 0) > 0]  # 过滤无数据项
    data["benchmarkLabels"] = [r.get("模型", "") for r in _bench_sorted]
    data["benchmarkIntel"] = [r.get("AA Intelligence Index", 0) or 0 for r in _bench_sorted]
    data["benchmarkCoding"] = [r.get("AA Coding Index", 0) or 0 for r in _bench_sorted]
  ...
  (function(){{
    ...
    if (isMobile) {{
      init('chart-benchmark-intel', {{
        ...
        tooltip:{{trigger:'axis', appendToBody:true}},
        legend:{{top:0,textStyle:{{color:muted}}}},
        grid:{{left:2,right:45,top:50,bottom:20,containLabel:true}},
        ...
        series:[
          ...
          {{name:'Coding Index',type:'bar',data:bCoding.map(function(v){{return v > bIntelMax ? null : v;}}),label:{{show:false}},itemStyle:{{borderRadius:[0,4,4,0]}}}}
        ]
      }});
    }} else {{
      init('chart-benchmark-intel', {{
        ...
        tooltip:{{trigger:'axis', appendToBody:true}},
        legend:{{top:0,textStyle:{{color:muted}}}},
        grid:{{left:70,right:30,top:56,bottom:80,containLabel:true}},
        ...
        series:[
          ...
          {{name:'Coding Index',type:'bar',data:bCoding,label:{{show:false}},itemStyle:{{borderRadius:[4,4,0,0]}}}}
        ]
      }});
    }}
  }})();
  // 图2: 散点图 - Cost per Task vs Intelligence（性价比象限图）
  (function(){{
    if (!DATA.benchmarkLabels || !DATA.benchmarkLabels.length) return;
    var scatterData = [];
    for (var i = 0; i < DATA.benchmarkLabels.length; i++) {{
      ...
      var coding = DATA.benchmarkCoding[i] || 10;
      if (cost > 0 && intel > 0) {{
        scatterData.push({{
          value:[cost, intel, coding],
          name:DATA.benchmarkLabels[i],
          symbolSize: Math.max(8, Math.min(40, coding / 3))
        }});
      }}
    }}
    ...
    if (isMobile) {{
      init('chart-benchmark-cost', {{
        animation:false,
        tooltip:{{trigger:'item', appendToBody:true, formatter:function(p){{return p.name + '<br/>Cost: $' + p.value[0] + '/Task<br/>Intelligence: ' + p.value[1] + '<br/>Coding: ' + p.value[2];}}}},
      ...
      init('chart-benchmark-cost', {{
        animation:false,
        tooltip:{{trigger:'item', appendToBody:true, formatter:function(p){{return p.name + '<br/>Cost per Task: $' + p.value[0] + '<br/>Intelligence Index: ' + p.value[1] + '<br/>Coding Index: ' + p.value[2];}}}},
