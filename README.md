...
2. 读取 `prompts/` 下的规则文件。
3. 采集、补采并校验数据（`scripts/discover_latest.py` 将当天实际采集到的动态来源写入 `data/discovered_gpu_{date}.json` 和 `data/discovered_token_{date}.json`）。
4. 运行 `scripts/generate_cmis_daily.py` 生成报告。
5. 校验图表脚本和关键输出文件。
6. 提交并推送生成结果。
7. 通过飞书 Webhook 发送通知（只含统计摘要和报告链接）。
...
│   ├── discovered_gpu_{YYYY-MM-DD}.json
│   ├── discovered_token_{YYYY-MM-DD}.json
│   ├── history.jsonl
...
- `scripts/discover_latest.py`：动态数据采集入口（写入 discovered_gpu / discovered_token）。
